...
## Description
codeasy school 是一個尋找程式教師的實作練習網頁。
透過 [https://mocki.io/](https://mocki.io/) 新增一個教師資訊列表 API，再串接此 API 取得教師資訊。
...
- [x] 應用 component 功能
- [x] 應用 computed 功能
- [x] 製作輸入類型 text、radio、select，同一 component 切換 prop 來改變對應的輸入類型，並套用 v-model
- [x] 可切換頁面
- [x] 設定頁面 title、description
- [x] 設定 Global 參數讓頁面的顏色主題會隨著更換
- [x] 串接一組列表資料 API
- [x] 程式架構做到最大的沿用性、維護性，並於Readme說明你的想法
...
<img width="600" alt="image" src="https://user-images.githubusercontent.com/56119287/229328506-507c91b4-262a-4a40-b362-91f3575ede00.png"> &nbsp; <img height="305" alt="image" src="https://user-images.githubusercontent.com/56119287/229333738-53e3b229-907e-4952-bbe8-c8b4ca0afe60.png">
  ...
  網頁首頁將網頁切分成左右兩部分，在手機和平板上做 RWD 切分成上下兩部分。<br>
  點擊按鈕會跳制搜尋教師的頁面。
...
將 Navigation Bar 放在 layouts 檔安中，之後新增其他頁面可以直接套用，不需要每一個頁面都寫入 header。<br>
後續也可以繼續在這個檔案增加 footer。

#### Theme color
  - 在 Navigation Bar 上提供四種不同顏色主題如下圖，依序為綠色、藍色、橙色、紫色。
  
    <img height="30" alt="image" src="https://user-images.githubusercontent.com/56119287/229336497-60253649-fdb8-41b7-aee5-b7be7a2b0168.png">
    
  - 透過 global variable 設定不同主題顏色 class，再使用 Vuex 控制主題狀態
  - 若是需增加主題，只要在 ./assets/css/variable.scss 中按照格式 (如下圖) 撰寫主題顏色，接著在 Navigation Bar 的頁面 (./layouts/default.vue) 增加顏色資料就可以直接套用。
...
搜尋頁面切分成上下區塊。上方區塊為搜尋版面，透過 text、radio、select 分別提供使用者不同的搜尋過濾條件 (搜尋內容文字、性別、教學地區)。下方區塊為教師資料列表版面，透過 computed 將 API 取得的教師列表資料處理成對應資訊內容，並且根據搜尋條件過濾資料。

#### Search Condition Pane (./components/search-condition-pane.vue)
  - 將各種搜尋條件類型包在此 component，透過 props 可以改變不同類型 (text、radio、select)。
  - 另外可以透過 v-model 雙向綁定資料，當使用者更改時父層 component 會依據新搜尋條件過濾顯示資料。
  - 考慮到可能需要綁定其他屬性到類型上 (比如：placeholder)，因此用 mainProps 直接綁定到類型上，增加擴充性。
  ...
  - 後續如果需要增加更多類型 (比如：checkbox)，可以繼續在此 component 中製作。若是類型種類過多可考慮將每個類型切分為獨立的 component，然後再 import 到此檔案中統一管理。

#### Teacher Card (./components/teacher-card.vue)
  - 將單一教師資訊版面包裝成 component，方便使用。
  - 透過 slot 讓大部分教師資料不需要用 props 傳遞到此 component，此 component 只需負責樣式的呈現和 rating star 資料處理，易於維護。
  - 以下為 HTML structure of teacher card，後續可以繼續新增教師其他欄位資訊 (比如：聯絡方式)，或是增加教師頁面連結按鈕。
...
#### Search text function optimization
  - 在搜尋文字功能上特別進行處理，盡可能過濾出相關教師資訊。
  - 搜尋文字可用 " " or "," 切分，例如 "script, python"，將會裁切成 "script" 和 "python"，只要任一個配對到資料內容皆會顯示該資料。
  - 透過 LCS algorithm 允許搜尋文字不完整的時也能盡量配對到資料，例如："pyhn" 可以配對到 python 相關的資料。
  - 只要 LCS 長度 >= 搜尋內容長度 * 0.8 皆能配對資料，允許部分搜尋文字有錯誤時也能盡量配對到資料，例如："puthon" 可以配對到 python 相關的資料。
  
### apis and utils directories
  - apis 資料夾將有串接的 api 進行統一管理，在 fetch 後接收完資料時回傳特定格式資料 (比如：JSON)。
  - utils 資料夾存放自己寫的額外功能檔案 (例如此專案所用的 LCS 演算法)。
